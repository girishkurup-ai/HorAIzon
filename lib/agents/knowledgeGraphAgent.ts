...
import { getEnv, createOpenAIClient } from '@/lib/config'

const getClient = () => createOpenAIClient()
const getModel = () => getEnv('CHAT_MODEL') || 'claude-opus-4.6-azure'
...
export async function updateKnowledgeGraph(
  ...
  const response = await getClient().chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are a Knowledge Graph Agent. Analyze a Q&A exchange and extract structured knowledge.
...
export async function generateTopicMarkup(
  ...
  const response = await getClient().chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are a Knowledge Documentation Agent. Synthesize interview knowledge into clean Markdown documentation.`,
      },
      {
        role: 'user',
        content: `Topic: "${topic}"
...
Format as clean, readable Markdown with headers.`,
  ...
  return response.choices[0]?.message?.content ?? rawMarkup
}
